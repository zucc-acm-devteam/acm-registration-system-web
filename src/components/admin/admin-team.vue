  ...
  <div v-loading="loading" v-if="isRefresh">
      ...
      <el-table :data="userList" style="width: 100%" :show-header="false">
        ...
        <el-table-column label="Value" align="center">
          <template slot-scope="scope">
            ...
            <template v-else-if="scope.row==='备注'">
              <markdown-it-vue :content="userShowData.remark?userShowData.remark:''"></markdown-it-vue>
            </template>
    ...
    <el-table :data="listData" style="width: 100%" :show-header="false">
      ...
      <el-table-column label="Members">
        <template slot-scope="scope">
          <template v-for="(item, index) of memberData.get(scope.row.id)">
            ...
            <el-button type="text" :key="item.username" style="color: black" @click="showUser(item.username)">
      ...
      <el-table-column label="Op" width="110">
        <template slot-scope="scope">
          <el-select v-model="scope.row.status" placeholder="请选择" size="small" style="width: 100px"
                     @change="saveTeam(scope.row)">
            <el-option label="等待审核" :value="1">
              <team-status :status="1"></team-status>
            </el-option>
            <el-option label="审核驳回" :value="2" :selected="true">
              <team-status :status="2"></team-status>
            </el-option>
            <el-option label="审核通过" :value="3">
              <team-status :status="3"></team-status>
            </el-option>
          </el-select>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getData (page) {
      ...
      let status = []
      for (let i = 1; i <= 3; i++) {
        if (this.showSetting[i]) status.push(i)
      }
      status = JSON.stringify(status)
      that.$http.get(that.$store.state.api + '/v1/team/?page_size=' + pageSize + '&contest_id=' + contest.id + '&page=' + page + '&status=' + status, auth)
        .then(data => {
